  ...
  return (
    <div className="p-5 min-h-screen flex items-start justify-center container mx-auto">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full relative bg-gradient-to-r from-indigo-100 to-blue-100">
        ...
        <Title level={2} className="text-center mb-8 text-blue-600 flex items-center justify-center gap-2">
        ...
        <div className="bg-gray-100 p-6 rounded-lg shadow-md mb-6 text-center">
          ...
          <div className="flex items-center justify-center gap-4 mt-4">
            ...
            <p className="text-lg font-semibold text-gray-700">
              {MatchInfo?.team1?.teamName} <span className="text-gray-400">vs</span> {MatchInfo?.team2?.teamName}
            </p>
            <img
          ...
          <div className="text-center bg-blue-50 p-4 rounded-lg shadow-lg">
            ...
            {status === "ongoing" && (
              <div className="flex gap-2 justify-center mt-2">
                ...
                <Button onClick={() => handleIncrement("team1", 2)} className="bg-blue-600 text-white rounded-md">
                  +2
                </Button>
                <Button onClick={() => handleIncrement("team1", 3)} className="bg-blue-600 text-white rounded-md">
                  +3
                </Button>
              </div>
            )}
          ...
          <div className="text-center bg-green-50 p-4 rounded-lg shadow-lg">
            ...
            {status === "ongoing" && (
              <div className="flex gap-2 justify-center mt-2">
                ...
                <Button onClick={() => handleIncrement("team2", 2)} className="bg-green-600 text-white rounded-md">
                  +2
                </Button>
                <Button onClick={() => handleIncrement("team2", 3)} className="bg-green-600 text-white rounded-md">
                  +3
                </Button>
              </div>
            )}
        ...
        {isCompleted && !isDraw && (
          <div className="text-center bg-yellow-50 p-4 rounded-lg shadow-md mb-6">
            ...
            <Text strong className="text-lg text-yellow-600">Match Completed</Text>
            <p className="mt-2">
              <span className="font-bold">{winner?.teamName}</span> wins over <span className="font-semibold">{loser?.teamName}</span>!
            </p>
          </div>
        )}
        ...
        {isCompleted && isDraw && (
          <div className="text-center bg-yellow-50 p-4 rounded-lg shadow-md mb-6">
            <CrownOutlined className="text-yellow-600 text-2xl" />
            <Text strong className="text-lg text-yellow-600">Match Completed</Text>
            <p className="mt-2">It's a draw between <span className="font-bold">{MatchInfo?.team1?.teamName}</span> and <span className="font-bold">{MatchInfo?.team2?.teamName}</span>!</p>
          </div>
        )}
